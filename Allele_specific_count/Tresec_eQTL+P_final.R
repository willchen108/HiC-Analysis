...
#remove all final p-values and ASE p-values = NA
results <- results[-which(is.na(x=results$final_Pvalue)),]
results <- results[-which(is.na(x=results$ASE_Pvalue)),]
require(qvalue)
results$final_Qvalue<-qvalue(results$final_Pvalue)$qvalues
#results_a$ASE_Qvalue<-qvalue(results_ase$ASE_Pvalue)$qvalues
...
require(ggplot2)
pdf('/net/shendure/vol10/projects/DNaseHiC.eQTLs/data/AScount/WASP/trecase_asl_qqplot_20160711.pdf',width=7*1.25,height=7)
