...
#SP loops
results = trecase(Y = trc, Y1 = ase1, Y2 = ase2, X = cov2, Z = genos, eChr = eChr, ePos = ePos, mChr = mChr, mPos = mPos, output.tag = 'AScount_SPloops_remapped_060616', p.cut = 1, local.only = TRUE, local.distance = 1, min.AS.sample = 2, min.n.het = 1)


results = read.delim('/net/shendure/vol10/projects/DNaseHiC.eQTLs/data/AScount/WASP_remapped/AScount_SPloops_remapped_060616_eqtl.txt', header=T)

results = read.delim('/Users/Will/Desktop/data/AScount/AScount_SPloops_remapped_060616_eqtl.txt', header=T)
