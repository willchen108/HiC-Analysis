...
# Keeps SNPs with 2 homozygous and 1 heterozygous
gt <- c(0, 1, 3, 4)
filter2 <- rep(TRUE,dim(genos)[2])
for (i in 1:dim(genos)[2]) {
  if (all(is.element(gt, genos[,i]))==FALSE){
    filter2[i] <- FALSE
    }
}
data <- data[which(filter2),]
...
results = read.delim('/net/shendure/vol10/projects/DNaseHiC.eQTLs/data/AScount/WASP_remapped/WASP_V3/AScount_eQTL+Promoter_realigned_060621_eqtl.txt', header=T)
...
require(ggplot2)
pdf('/net/shendure/vol10/projects/DNaseHiC.eQTLs/data/AScount/WASP_remapped/WASP_V3/trecase_asl_qqplot_20160621.pdf',width=7*1.25,height=7)
gg_qqplot(na.omit(results$final_Pvalue))
dev.off()

require(ggplot2)
pdf('/net/shendure/vol10/projects/DNaseHiC.eQTLs/data/AScount/WASP_remapped/WASP_V3/trecase_asl_qqplot_filtered_20160622.pdf',width=7*1.25,height=7)
...
# Plot box plot
growIDs <- results$GeneRowID[which(results$final_Pvalue<1.0e-4)]
projdirs <- '/net/shendure/vol10/projects/DNaseHiC.eQTLs/data/AScount/WASP_remapped/WASP_V3/box_plot/'
...
growIDs <- results_ase$GeneRowID[which(results_ase$final_Pvalue<1.0e-4)]
projdirs <- '/net/shendure/vol10/projects/DNaseHiC.eQTLs/data/AScount/WASP_remapped/WASP_V3/box_plot/'
for (i in 1:length(growIDs)) 
  {
  growID <- growIDs[i] 
  ...
  ref <- data[growID,4]
  alt <- data[growID,5]
  results_ase[which(results_ase$GeneRowID==growID),]
  cmax <- max(trc[,growID]/ratio)
  ylim <- 10^floor(log10(cmax)) * (round(cmax/10^floor(log10(cmax)))+1)
  x1 <- (trc[,growID]/ratio)[which(geno==0)]
  x2 <- (trc[,growID]/ratio)[which(geno==1)]
  ...
  points(jittergeno,as.numeric(trc[,growID]/ratio), col="grey25",pch=16,bty='n',cex=1.25)
  axis(1,at=c(1:3),labels=c(paste0(ref,"/",ref,"(ref/ref) n = ",toString(sum(geno==0))),paste0(ref,"/",alt,"(ref/alt) n = ",toString(sum(geno==1))),paste0(alt,"/",alt,"(alt/alt) n = ",toString(sum(geno==2)))),lty=0,cex.axis=1.25)
  pvalue <- toString(results_ase[which(results_ase$GeneRowID==growID),][20])
  text(1,10,paste0("P = ",pvalue))
  dev.off()
}
